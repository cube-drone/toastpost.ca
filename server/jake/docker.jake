 ...
 *             ports: [1337])
 */
async function start ({name, network, container, hostname, environment, detached, ports, volumes, complexArgs, command, yes}) {
    if(await isRunning({name: name})){
    ...
    if(await isThere({name: name})){
        console.log(`${name} is still around, but dead`);
        await stop({name});
    }
    ...
    let volumeString = "";
    if(volumes != null){
        volumeString = Object.keys(volumes).map((volume) => {
            return `-v "${volume}:${volumes[volume]}"`;            
        }).join(" ");
    }

    if(detached) {
        let cmd = `docker run -d --log-driver=json-file ${hostnameString} ${volumeString} ${networkString} ${portString} ${environmentString} --name=${name} ${complexArgs || ""} ${container} ${command || ""}`;
        console.log(cmd);
        return run(cmd);
    } else {
        let cmd =`docker run -i --rm ${hostnameString} ${volumeString} ${networkString} ${portString} ${environmentString} --name=${name} ${complexArgs || ""} ${container} ${command || ""}`;
        console.log(cmd);
...
module.exports = {createNetwork, getIp, runningList, isRunning, start, stop};
